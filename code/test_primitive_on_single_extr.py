...
def main(args):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataloader):
            ...
            #Use the input point clouds from "from_vec", as model was trained on these
            # and not the faulty ones from the original repo
            try:
                pc_path = os.path.join(DATA_DIR, "pc_from_vec", id[:4], id + ".ply")
                pc = o3d.io.read_point_cloud(pc_path)
                pc = torch.tensor(np.asarray(pc.points), dtype = torch.float32)
                sample_idx = random.sample(list(range(pc.shape[0])), 2048)
                pc = pc[sample_idx]
                pc = pc.unsqueeze(0)  # Add batch dimension
            except Exception as e:
                missing_pc.append(id)
                continue
