...
def main(args):
    ...
    with torch.no_grad(): # REMOVE DATALOADER
        for i, data_complex in enumerate(test_complex_dataset):
            ...
            if len(parts_indices) == 1: # only infer complex models
                simple_model_counter += 1
                continue
    ...
    print(f"Total complex models inferred: {len(test_complex_dataset) - simple_model_counter} out of {len(test_complex_dataset)} samples", flush=True)

    df = pd.DataFrame(rows, columns=cols)
    df.to_pickle(os.path.join(model_dir, "test_results_SEM_complex.pkl"))
